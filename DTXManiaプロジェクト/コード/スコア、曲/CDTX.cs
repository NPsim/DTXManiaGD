	...
	internal class CDTX : CActivity
	{
		...
		public class CChip : IComparable<CDTX.CChip>, ICloneable
		{
			...
			public int GetDuration()
			{
				...
				{
					CDTX.CWAV wc;
					CDTXMania.DTX.listWAV.TryGetValue( this.n整数値_KMD_内部番号, out wc );
					if ( wc == null )
					{
						nDuration = 0;
					}
...
//		public bool bBASS演奏で直前のBASSを消音する;
		...
		{
			this.On活性化();
			this.t入力_KMD_全入力文字列から( str全入力文字列 );
		}
		...
		{
			this.On活性化();
			this.t入力_KMD_全入力文字列から( str全入力文字列, db再生速度, nBGMAdjust );
		}
		...
		#region [ BMP/BMPTEXの並列読み込み_KMD_デコード用メソッド ]
		...
		public void tBMP_BMPTEXの読み込み()
		{
			...
			if ( this.listBMP != null )
			{
				if ( nCPUCores <= 1 )
				{
					#region [ シングルスレッドで逐次読み出し_KMD_デコード_KMD_テクスチャ定義 ]
				...
				{
					#region [ メインスレッド(テクスチャ定義)とバックグラウンドスレッド(読み出し_KMD_デコード)を並列動作させ高速化 ]
        ...
        public void tドラムのランダム化(E楽器パート part, Eランダムモード eRandom)
        {
            ...
            else if (part == E楽器パート.DRUMS && eRandom != Eランダムモード.OFF)
            {
                ...
                int num9 = -10000;
                int[] n乱数排列数列;
                CDTX.t乱数排列数列生成作業_KMD_クラシック(out n乱数排列数列);
                foreach (CDTX.CChip current2 in this.listChip)
                {
                    ...
                    if (part == E楽器パート.DRUMS && 17 <= nチャンネル番号2 && nチャンネル番号2 <= 24 && nチャンネル番号2 != 19)
                    {
                        ...
                        {
                            case Eランダムモード.RANDOM:
                                CDTX.t乱数を各チャンネルに指定する_KMD_クラシック(n乱数排列数列, current2, nチャンネル番号2);
                                break;
                            case Eランダムモード.SUPERRANDOM:
        ...
        private static void t乱数を各チャンネルに指定する_KMD_クラシック(int[] n乱数排列数列, CDTX.CChip chip, int nランダム化前チャンネル番号)
        {
            switch (nランダム化前チャンネル番号)
            {
                case 0x11:
        ...
        private static void t乱数排列数列生成作業_KMD_クラシック(out int[] n乱数排列数列)
        {
            n乱数排列数列 = new int[5];
...
//					GOTO_END++;		// goto用のダミーコード
		...
		{
			if( pChip.n整数値_KMD_内部番号 >= 0 )
			{
				if( ( nLane < (int) Eレーン.LC ) || ( (int) Eレーン.BGM < nLane ) )
				{
					throw new ArgumentOutOfRangeException();
				}
				if( this.listWAV.ContainsKey( pChip.n整数値_KMD_内部番号 ) )
				{
					CWAV wc = this.listWAV[ pChip.n整数値_KMD_内部番号 ];
		...
		public void t入力( string strファイル名, bool bヘッダのみ, double db再生速度, int nBGMAdjust )
		{
			...
			if ( this.e種別 != E種別.SMF )
			{
				try
				{
					...
					this.t入力_KMD_全入力文字列から( str2, db再生速度, nBGMAdjust );
				}
				catch
				{
				}
		...
		public void t入力_KMD_全入力文字列から( string str全入力文字列 )
		{
			this.t入力_KMD_全入力文字列から( str全入力文字列, 1.0, 0 );
		}
		public unsafe void t入力_KMD_全入力文字列から( string str全入力文字列, double db再生速度, int nBGMAdjust )
		{
			//DateTime timeBeginLoad = DateTime.Now;
			//TimeSpan span;
			...
			if ( !string.IsNullOrEmpty( str全入力文字列 ) )
			{
				...
				if ( ce.MoveNext() )
				{
					...
					do
					{
						if ( !this.t入力_KMD_空白と改行をスキップする( ref ce ) )
						{
							break;
						}
						if ( ce.Current == '#' )
						{
							if ( ce.MoveNext() )
							{
								...
								if ( this.t入力_KMD_コマンド文字列を抜き出す( ref ce, ref builder ) )
								{
									StringBuilder builder2 = new StringBuilder( 0x400 );
									if ( this.t入力_KMD_パラメータ文字列を抜き出す( ref ce, ref builder2 ) )
									{
										StringBuilder builder3 = new StringBuilder( 0x400 );
										if ( this.t入力_KMD_コメント文字列を抜き出す( ref ce, ref builder3 ) )
										{
											this.t入力_KMD_行解析( ref builder, ref builder2, ref builder3 );
											this.n現在の行数++;
											continue;
										}
									}
					...
					while ( this.t入力_KMD_コメントをスキップする( ref ce ) );
				#endregion
					//span = (TimeSpan) ( DateTime.Now - timeBeginLoad );
					...
					if ( !this.bヘッダのみ )
					{
						...
						foreach ( CChip chip in this.listChip )
						{
							if ( this.listWAV.ContainsKey( chip.n整数値_KMD_内部番号 ) )
							{
								CWAV cwav = this.listWAV[ chip.n整数値_KMD_内部番号 ];
						...
						//{
							...
							//{
								...
								//{
									...
									//c.n整数値 = chip.n整数値;
									//c.n整数値_KMD_内部番号 = chip.n整数値_KMD_内部番号;
						...
						#endregion
						//span = (TimeSpan) ( DateTime.Now - timeBeginLoad );
						//Trace.TraceInformation( "拍子_KMD_拍線挿入時間:       {0}", span.ToString() );
						//timeBeginLoad = DateTime.Now;
						#region [ C2 [拍線_KMD_小節線表示指定] の処理 ]		// #28145 2012.4.21 yyagi; 2重ループをほぼ1重にして高速化
						...
						#endregion
						//span = (TimeSpan) ( DateTime.Now - timeBeginLoad );
						//Trace.TraceInformation( "C2 [拍線_KMD_小節線表示指定]:  {0}", span.ToString() );
						//timeBeginLoad = DateTime.Now;
						#region [ 発声時刻の計算 ]
						double bpm = 120.0;
								...
								case 0x08:	// 拡張BPM
									{
										n発声位置 = chip.n発声位置;
										ms = chip.n発声時刻ms;
										if ( this.listBPM.ContainsKey( chip.n整数値_KMD_内部番号 ) )
										{
											bpm = ( ( this.listBPM[ chip.n整数値_KMD_内部番号 ].n表記上の番号 == 0 ) ? 0.0 : this.BASEBPM ) + this.listBPM[ chip.n整数値_KMD_内部番号 ].dbBPM値;
										}
										continue;
									}
								case 0x54:	// 動画再生
                        ...
                        foreach (CChip chip in this.listChip)
                        {
                            if ((chip.bWAVを使うチャンネルである && this.listWAV.ContainsKey(chip.n整数値_KMD_内部番号)) && !this.listWAV[chip.n整数値_KMD_内部番号].listこのWAVを使用するチャンネル番号の集合.Contains(chip.nチャンネル番号))
                            {
                                this.listWAV[chip.n整数値_KMD_内部番号].listこのWAVを使用するチャンネル番号の集合.Add(chip.nチャンネル番号);

                                int c = chip.nチャンネル番号 >> 4;
                                switch (c)
                                {
                                    case 0x01:
                                        this.listWAV[chip.n整数値_KMD_内部番号].bIsDrumsSound = true; break;
                                    case 0x02:
                                        this.listWAV[chip.n整数値_KMD_内部番号].bIsGuitarSound = true; break;
                                    case 0x0A:
                                        this.listWAV[chip.n整数値_KMD_内部番号].bIsBassSound = true; break;
                                    case 0x06:
                                    case 0x07:
                                    case 0x08:
                                    case 0x09:
                                        this.listWAV[chip.n整数値_KMD_内部番号].bIsSESound = true; break;
                                    case 0x00:
                                        if (chip.nチャンネル番号 == 0x01)
                                        {
                                            this.listWAV[chip.n整数値_KMD_内部番号].bIsBGMSound = true; break;
                                        }
                                        break;
                                }
                            }
        ...
        /// <summary>
	    /// サウンドミキサーにサウンドを登録_KMD_削除する時刻を事前に算出する
	    /// </summary>
			...
			foreach ( CChip pChip in listChip )
			{
				switch ( pChip.nチャンネル番号 )
				{
					...
					case 0x90:	case 0x91:	case 0x92:
						...
						{
							if ( listWAV.ContainsKey( pChip.n整数値_KMD_内部番号 ) )
							{
								CDTX.CWAV wc = CDTXMania.DTX.listWAV[ pChip.n整数値_KMD_内部番号 ];
								if ( wc.rSound[ 0 ] != null )
								{
...
//Debug.WriteLine( "nAddMixer時刻ms=" + nAddMixer時刻ms + ",nAddMixer位置=" + nAddMixer位置 );
						...
						CChip c_AddMixer = new CChip()
						{
							nチャンネル番号 = 0xEA,
							n整数値 = pChip.n整数値,
							n整数値_KMD_内部番号 = pChip.n整数値_KMD_内部番号,
...
//Debug.WriteLine( "n新RemoveMixer時刻ms=" + n新RemoveMixer時刻ms + ",n新RemoveMixer位置=" + n新RemoveMixer位置 );
						if ( n新RemoveMixer時刻ms < pChip.n発声時刻ms + duration )	// 曲の最後でサウンドが切れるような場合は
						{
	                        ...
	                        //continue;                 // 発声位置の計算ができないので、Mixer削除をあきらめる_KMD__KMD__KMD_のではなく
...
//DebugOut_CChipList( listRemoveMixerChannel );
							...
							CChip c = new CChip()											// mixer削除時刻を更新(遅延)する
							{
								...
								n整数値_KMD_内部番号 = listRemoveTiming[ index ].n整数値_KMD_内部番号,
		...
		private bool t発声時刻msと発声位置を取得する( int n希望発声時刻ms, out int n新発声時刻ms, out int n新発声位置 )
		{
			...
			if ( index_min < 0 )	// 希望発声時刻に至らずに曲が終了してしまう場合
			{
				// listの最終項目の時刻をそのまま使用する
								//_KMD__KMD__KMD_のではダメ。BGMが尻切れになる。
...
//			SwapGuitarBassInfos_AutoFlags();
		...
		private bool t入力_KMD_コマンド文字列を抜き出す( ref CharEnumerator ce, ref StringBuilder sb文字列 )
		{
			if( !this.t入力_KMD_空白をスキップする( ref ce ) )
				return false;	// 文字が尽きた
		...
		private bool t入力_KMD_コメントをスキップする( ref CharEnumerator ce )
		{
		...
		private bool t入力_KMD_コメント文字列を抜き出す( ref CharEnumerator ce, ref StringBuilder sb文字列 )
		{
		...
		private void t入力_KMD_パラメータ食い込みチェック( string strコマンド名, ref string strコマンド, ref string strパラメータ )
		{
		...
		private bool t入力_KMD_パラメータ文字列を抜き出す( ref CharEnumerator ce, ref StringBuilder sb文字列 )
		{
			if( !this.t入力_KMD_空白をスキップする( ref ce ) )
				return false;	// 文字が尽きた
		...
		private bool t入力_KMD_空白と改行をスキップする( ref CharEnumerator ce )
		{
		...
		private bool t入力_KMD_空白をスキップする( ref CharEnumerator ce )
		{
			// 空白が続く間はこれをスキップする。
		...
		private void t入力_KMD_行解析( ref StringBuilder sbコマンド, ref StringBuilder sbパラメータ, ref StringBuilder sbコメント )
		{
			...
			{
				this.t入力_KMD_パラメータ食い込みチェック( "IF", ref strコマンド, ref strパラメータ );
			...
			{
				this.t入力_KMD_パラメータ食い込みチェック( "ENDIF", ref strコマンド, ref strパラメータ );
			...
			else if( !this.bstackIFからENDIFをスキップする.Peek() )		// IF～ENDIF をスキップするなら以下はすべて無視。
			{
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "PATH_WAV", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "PATH", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "TITLE", ref strコマンド, ref strパラメータ );
					this.TITLE = strパラメータ;
				}
				//-----------------
				#endregion
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "ARTIST", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "COMMENT", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "DLEVEL", ref strコマンド, ref strパラメータ );
					this.t入力_KMD_パラメータ食い込みチェック( "PLAYLEVEL", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "GLEVEL", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "BLEVEL", ref strコマンド, ref strパラメータ );
                ...
                {
                    this.t入力_KMD_パラメータ食い込みチェック( "DLVDEC", ref strコマンド, ref strパラメータ);
                    int dleveldec;
                ...
                {
                    this.t入力_KMD_パラメータ食い込みチェック( "GLVDEC", ref strコマンド, ref strパラメータ);
                    int gleveldec;
                ...
                {
                    this.t入力_KMD_パラメータ食い込みチェック( "BLVDEC", ref strコマンド, ref strパラメータ);
                    int bleveldec;
					...
					this.t入力_KMD_パラメータ食い込みチェック("SUPRESSNOTEOFF_HIHAT", ref str, ref str2);
					...
					this.t入力_KMD_パラメータ食い込みチェック("SUPRESSNOTEOFF_GUITAR", ref str, ref str2);
					...
					this.t入力_KMD_パラメータ食い込みチェック("SUPRESSNOTEOFF_BASS", ref str, ref str2);
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "GENRE", ref strコマンド, ref strパラメータ );
					this.GENRE = strパラメータ;
				}
				//-----------------
				#endregion
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "HIDDENLEVEL", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "STAGEFILE", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "PREVIEW", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "PREIMAGE", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "PREMOVIE", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "BACKGROUND_GR", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "BACKGROUND", ref strコマンド, ref strパラメータ );
					this.t入力_KMD_パラメータ食い込みチェック( "WALL", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "RANDOM", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "SOUND_NOWLOADING", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_行解析_KMD_BPM_BPMzz( strコマンド, strパラメータ, strコメント );
				}
				//-----------------
				#endregion
                else if( strコマンド.StartsWith( "FORCINGXG"  ) )
                {
                    this.t入力_KMD_パラメータ食い込みチェック( "FORCINGXG", ref strコマンド, ref strパラメータ );
					this.b強制的にXG譜面にする = strパラメータ.ToLower().Equals( "on" );
                }
                else if( strコマンド.StartsWith( "VOL7FTO64" ) )
                {
                    this.t入力_KMD_パラメータ食い込みチェック( "VOL7FTO64", ref strコマンド, ref strパラメータ );
				...
				{
					this.t入力_KMD_パラメータ食い込みチェック( "DTXVPLAYSPEED", ref strコマンド, ref strパラメータ );
				...
				else if( !this.bヘッダのみ )		// ヘッダのみの解析の場合、以下は無視。
				{
					...
					{
						this.t入力_KMD_パラメータ食い込みチェック( "PANEL", ref strコマンド, ref strパラメータ );
					...
					{
						this.t入力_KMD_パラメータ食い込みチェック( "MIDIFILE", ref strコマンド, ref strパラメータ );
					...
					{
						this.t入力_KMD_パラメータ食い込みチェック( "MIDINOTE", ref strコマンド, ref strパラメータ );
					...
					{
						this.t入力_KMD_パラメータ食い込みチェック( "BLACKCOLORKEY", ref strコマンド, ref strパラメータ );
					...
					{
						this.t入力_KMD_パラメータ食い込みチェック( "BASEBPM", ref strコマンド, ref strパラメータ );
					...
					{
						this.t入力_KMD_パラメータ食い込みチェック( "SOUND_STAGEFAILED", ref strコマンド, ref strパラメータ );
					...
					{
						this.t入力_KMD_パラメータ食い込みチェック( "SOUND_FULLCOMBO", ref strコマンド, ref strパラメータ );
					...
					{
						this.t入力_KMD_パラメータ食い込みチェック( "SOUND_AUDIENCE", ref strコマンド, ref strパラメータ );
					...
					else if( !this.t入力_KMD_行解析_KMD_WAVVOL_VOLUME( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_WAVPAN_PAN( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_WAV( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_BMPTEX( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_BMP( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_BGAPAN( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_BGA( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_AVIPAN( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_AVI_VIDEO( strコマンド, strパラメータ, strコメント ) &&
					//	!this.t入力_KMD_行解析_KMD_BPM_BPMzz( strコマンド, strパラメータ, strコメント ) &&	// bヘッダのみ==trueの場合でもチェックするよう変更
						!this.t入力_KMD_行解析_KMD_RESULTIMAGE( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_RESULTMOVIE( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_RESULTSOUND( strコマンド, strパラメータ, strコメント ) &&
						!this.t入力_KMD_行解析_KMD_SIZE( strコマンド, strパラメータ, strコメント ) )
					{
						this.t入力_KMD_行解析_KMD_チップ配置( strコマンド, strパラメータ, strコメント );
					}
				EOL:
		...
		private bool t入力_KMD_行解析_KMD_AVI_VIDEO( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
		...
		private bool t入力_KMD_行解析_KMD_AVIPAN( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
			...
			//-----------------
			#endregion
			#region [ 2. 開始転送サイズ_KMD_幅 ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（開始転送サイズ_KMD_幅）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 3. 転送サイズ_KMD_高さ ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（開始転送サイズ_KMD_高さ）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 4. 終了転送サイズ_KMD_幅 ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（終了転送サイズ_KMD_幅）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 5. 終了転送サイズ_KMD_高さ ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（終了転送サイズ_KMD_高さ）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 6. 動画側開始位置_KMD_X ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（動画側開始位置_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 7. 動画側開始位置_KMD_Y ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（動画側開始位置_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 8. 動画側終了位置_KMD_X ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（動画側終了位置_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 9. 動画側終了位置_KMD_Y ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（動画側終了位置_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 10.表示側開始位置_KMD_X ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（表示側開始位置_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 11.表示側開始位置_KMD_Y ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（表示側開始位置_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 12.表示側終了位置_KMD_X ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（表示側終了位置_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 13.表示側終了位置_KMD_Y ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "AVIPAN: {2}番目の引数（表示側終了位置_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
		...
		private bool t入力_KMD_行解析_KMD_BGA( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
			...
			//-----------------
			#endregion
			#region [ 2.画像側位置１_KMD_X ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGA: {2}番目の引数（画像側位置１_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			bga.pt画像側左上座標.X = n値;
			i++;
			//-----------------
			#endregion
			#region [ 3.画像側位置１_KMD_Y ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGA: {2}番目の引数（画像側位置１_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			bga.pt画像側左上座標.Y = n値;
			i++;
			//-----------------
			#endregion
			#region [ 4.画像側位置２_KMD_X ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGA: {2}番目の引数（画像側位置２_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			bga.pt画像側右下座標.X = n値;
			i++;
			//-----------------
			#endregion
			#region [ 5.画像側位置２_KMD_Y ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGA: {2}番目の引数（画像側座標２_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			bga.pt画像側右下座標.Y = n値;
			i++;
			...
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGA: {2}番目の引数（表示位置_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			bga.pt表示座標.X = n値;
			i++;
			...
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGA: {2}番目の引数（表示位置_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			bga.pt表示座標.Y = n値;
			i++;
		...
		private bool t入力_KMD_行解析_KMD_BGAPAN( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
			...
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（開始転送サイズ_KMD_幅）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 3. 開始転送サイズ_KMD_高さ ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（開始転送サイズ_KMD_高さ）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（終了転送サイズ_KMD_幅）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（終了転送サイズ_KMD_高さ）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 6. 画像側開始位置_KMD_X ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（画像側開始位置_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 7. 画像側開始位置_KMD_Y ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（画像側開始位置_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 8. 画像側終了位置_KMD_X ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（画像側終了位置_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			//-----------------
			#endregion
			#region [ 9. 画像側終了位置_KMD_Y ]
			//-----------------
			n値 = 0;
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（画像側終了位置_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（表示側開始位置_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（表示側開始位置_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（表示側終了位置_KMD_X）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
			...
			if( !int.TryParse( strParams[ i ], out n値 ) )
			{
				Trace.TraceError( "BGAPAN: {2}番目の引数（表示側終了位置_KMD_Y）が異常です。[{0}: {1}行]", this.strファイル名の絶対パス, this.n現在の行数, i + 1 );
				return false;
			}
		...
		private bool t入力_KMD_行解析_KMD_BMP( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
		...
		private bool t入力_KMD_行解析_KMD_BMPTEX( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
		...
		private bool t入力_KMD_行解析_KMD_BPM_BPMzz( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
			...
			if( this.n無限管理BPM[ zz ] == -zz )	// 初期状態では n無限管理BPM[zz] = -zz である。この場合、#BPMzz がまだ出現していないことを意味する。
			{
				for( int i = 0; i < this.listChip.Count; i++ )	// これまでに出てきたチップのうち、該当する（BPM値が未設定の）BPMチップの値を変更する（仕組み上、必ず後方参照となる）。
				{
					...
					if( chip.bBPMチップである && chip.n整数値_KMD_内部番号 == -zz )	// #BPMzz 行より前の行に出現した #BPMzz では、整数値_KMD_内部番号は -zz に初期化されている。
						chip.n整数値_KMD_内部番号 = this.n内部番号BPM1to;
				}
			}
		...
		private bool t入力_KMD_行解析_KMD_RESULTIMAGE( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
			...
			else
			{
				...
				{
					case "_SS":
						this.t入力_KMD_行解析_KMD_RESULTIMAGE_KMD_ファイルを設定する( 0, strパラメータ );
						break;

					case "_S":
						this.t入力_KMD_行解析_KMD_RESULTIMAGE_KMD_ファイルを設定する( 1, strパラメータ );
						break;

					case "_A":
						this.t入力_KMD_行解析_KMD_RESULTIMAGE_KMD_ファイルを設定する( 2, strパラメータ );
						break;

					case "_B":
						this.t入力_KMD_行解析_KMD_RESULTIMAGE_KMD_ファイルを設定する( 3, strパラメータ );
						break;

					case "_C":
						this.t入力_KMD_行解析_KMD_RESULTIMAGE_KMD_ファイルを設定する( 4, strパラメータ );
						break;

					case "_D":
						this.t入力_KMD_行解析_KMD_RESULTIMAGE_KMD_ファイルを設定する( 5, strパラメータ );
						break;

					case "_E":
						this.t入力_KMD_行解析_KMD_RESULTIMAGE_KMD_ファイルを設定する( 6, strパラメータ );
						break;
				}
				//-----------------
				#endregion
		...
		private void t入力_KMD_行解析_KMD_RESULTIMAGE_KMD_ファイルを設定する( int nランク0to6, string strファイル名 )
		{
		...
		private bool t入力_KMD_行解析_KMD_RESULTMOVIE( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
			...
			else
			{
				...
				{
					case "_SS":
						this.t入力_KMD_行解析_KMD_RESULTMOVIE_KMD_ファイルを設定する( 0, strパラメータ );
						break;

					case "_S":
						this.t入力_KMD_行解析_KMD_RESULTMOVIE_KMD_ファイルを設定する( 1, strパラメータ );
						break;

					case "_A":
						this.t入力_KMD_行解析_KMD_RESULTMOVIE_KMD_ファイルを設定する( 2, strパラメータ );
						break;

					case "_B":
						this.t入力_KMD_行解析_KMD_RESULTMOVIE_KMD_ファイルを設定する( 3, strパラメータ );
						break;

					case "_C":
						this.t入力_KMD_行解析_KMD_RESULTMOVIE_KMD_ファイルを設定する( 4, strパラメータ );
						break;

					case "_D":
						this.t入力_KMD_行解析_KMD_RESULTMOVIE_KMD_ファイルを設定する( 5, strパラメータ );
						break;

					case "_E":
						this.t入力_KMD_行解析_KMD_RESULTMOVIE_KMD_ファイルを設定する( 6, strパラメータ );
						break;
				}
				//-----------------
				#endregion
		...
		private void t入力_KMD_行解析_KMD_RESULTMOVIE_KMD_ファイルを設定する( int nランク0to6, string strファイル名 )
		{
			if( nランク0to6 < 0 || nランク0to6 > 6 )	// 値域チェック。
				return;
		...
		private bool t入力_KMD_行解析_KMD_RESULTSOUND( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
			...
			else
			{
				...
				{
					case "_SS":
						this.t入力_KMD_行解析_KMD_RESULTSOUND_KMD_ファイルを設定する( 0, strパラメータ );
						break;

					case "_S":
						this.t入力_KMD_行解析_KMD_RESULTSOUND_KMD_ファイルを設定する( 1, strパラメータ );
						break;

					case "_A":
						this.t入力_KMD_行解析_KMD_RESULTSOUND_KMD_ファイルを設定する( 2, strパラメータ );
						break;

					case "_B":
						this.t入力_KMD_行解析_KMD_RESULTSOUND_KMD_ファイルを設定する( 3, strパラメータ );
						break;

					case "_C":
						this.t入力_KMD_行解析_KMD_RESULTSOUND_KMD_ファイルを設定する( 4, strパラメータ );
						break;

					case "_D":
						this.t入力_KMD_行解析_KMD_RESULTSOUND_KMD_ファイルを設定する( 5, strパラメータ );
						break;

					case "_E":
						this.t入力_KMD_行解析_KMD_RESULTSOUND_KMD_ファイルを設定する( 6, strパラメータ );
						break;
				}
				//-----------------
				#endregion
		...
		private void t入力_KMD_行解析_KMD_RESULTSOUND_KMD_ファイルを設定する( int nランク0to6, string strファイル名 )
		{
			if( nランク0to6 < 0 || nランク0to6 > 6 )	// 値域チェック。
				return;
		...
		private bool t入力_KMD_行解析_KMD_SIZE( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
		...
		private bool t入力_KMD_行解析_KMD_WAV( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
			...
			if( this.n無限管理WAV[ zz ] == -zz )	// 初期状態では n無限管理WAV[zz] = -zz である。この場合、#WAVzz がまだ出現していないことを意味する。
			{
				for( int i = 0; i < this.listChip.Count; i++ )	// これまでに出てきたチップのうち、該当する（内部番号が未設定の）WAVチップの値を変更する（仕組み上、必ず後方参照となる）。
				{
					...
					if( chip.bWAVを使うチャンネルである && ( chip.n整数値_KMD_内部番号 == -zz ) )	// この #WAVzz 行より前の行に出現した #WAVzz では、整数値_KMD_内部番号は -zz に初期化されている。
						chip.n整数値_KMD_内部番号 = this.n内部番号WAV1to;
				}
			}
		...
		private bool t入力_KMD_行解析_KMD_WAVPAN_PAN( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
		...
		private bool t入力_KMD_行解析_KMD_WAVVOL_VOLUME( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
		...
		private bool t入力_KMD_行解析_KMD_チップ配置( string strコマンド, string strパラメータ, string strコメント )
		{
			// (1) コマンドを処理。
			...
			for( int i = 0; i < ( n文字数 / 2 ); i++ )	// 2文字で1オブジェクト数値
			{
				...
				{
					chip.n整数値_KMD_内部番号 = this.n無限管理WAV[ nオブジェクト数値 ];	// これが本当に一意なWAV番号となる。（無限定義の場合、chip.n整数値 は一意である保証がない。）
				}
				else if( chip.bBPMチップである )
				{
					chip.n整数値_KMD_内部番号 = this.n無限管理BPM[ nオブジェクト数値 ];	// これが本当に一意なBPM番号となる。（同上。）
				}
				//-----------------
				#endregion
		...
		private bool TryParse(string s, out double result) {	// #23880 2010.12.30 yyagi: alternative TryParse to permit both '.' and ',' for decimal point
			...
			for (int i = 0; i < len; i++) {							// まず、真の小数点(一番最後に現れる小数点)の位置を求める
				...
				if (NumberSymbols.IndexOf(c) >= 0) {				// 数値だったらスキップ
					...
					continue;
				} else {											// 数値_KMD_小数点_KMD_区切り文字以外がきたらループ終了
					break;
				}
			}
