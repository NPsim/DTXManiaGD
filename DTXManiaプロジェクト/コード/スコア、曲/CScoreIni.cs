...
namespace DTXMania
{
	public class CScoreIni
	{
		...
		public class C演奏記録
		{
			...
			public int nPoor数;
			public int nPerfect数_KMD_Auto含まない;
			public int nGreat数_KMD_Auto含まない;
			public int nGood数_KMD_Auto含まない;
			public int nPoor数_KMD_Auto含まない;
			public int nMiss数_KMD_Auto含まない;
			public long nスコア;
			public int n演奏速度分子;
			public int n演奏速度分母;
			public int n最大コンボ数;
			...
			public bool b全AUTOである
			{
				get
				{
					return (this.n全チップ数 - this.nPerfect数_KMD_Auto含まない - this.nGreat数_KMD_Auto含まない - this.nGood数_KMD_Auto含まない - this.nPoor数_KMD_Auto含まない - this.nMiss数_KMD_Auto含まない) == this.n全チップ数;
				}
			}
#if false
			[StructLayout( LayoutKind.Sequential )]
        ...
        {
            //こちらはプレイヤースキル_KMD_全曲スキルに加算される得点。いわゆる曲別スキル。
        ...
        internal static int t総合ランク値を計算して返す(C演奏記録 Drums, C演奏記録 Guitar, C演奏記録 Bass)
        {
            ...
            int nPerfect = Drums.nPerfect数_KMD_Auto含まない + Guitar.nPerfect数_KMD_Auto含まない + Bass.nPerfect数_KMD_Auto含まない;	// #24569 2011.3.1 yyagi: to calculate result rank without AUTO chips
            int nGreat = Drums.nGreat数_KMD_Auto含まない + Guitar.nGreat数_KMD_Auto含まない + Bass.nGreat数_KMD_Auto含まない;		//
            int nGood = Drums.nGood数_KMD_Auto含まない + Guitar.nGood数_KMD_Auto含まない + Bass.nGood数_KMD_Auto含まない;		//
            int nPoor = Drums.nPoor数_KMD_Auto含まない + Guitar.nPoor数_KMD_Auto含まない + Bass.nPoor数_KMD_Auto含まない;		//
            int nMiss = Drums.nMiss数_KMD_Auto含まない + Guitar.nMiss数_KMD_Auto含まない + Bass.nMiss数_KMD_Auto含まない;		//
