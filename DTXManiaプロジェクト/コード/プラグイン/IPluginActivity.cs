...
namespace DTXMania
{
	...
	/// <para>(1) DTXMania の起動処理の最後（セットアップ画面が表示される直前）に一度だけ、On初期化()_KMD_OnManagedリソースの作成()_KMD_OnUnmanagedリソースの作成() をこの順番で1回ずつ呼び出します。</para>
	/// <para>(2) DTXMania の終了処理の最初（Thank you for playing が消えた直後）に一度だけ、OnUnmanagedリソースの解放()_KMD_OnManagedリソースの解放()_KMD_On終了() をこの順番で1回ずつ呼び出します。</para>
	...
	/// <para>(5) Direct3D デバイスのロスト時には、デバイスの再生成前に OnUnmanagedリソースの解放()_KMD_OnManagedリソースの解放() を1回ずつ呼び出し、デバイスを再生成したのち、OnManagedリソースの作成()_KMD_OnUnmanagedリソースの作成() を1回ずつ呼び出します。</para>
	/// </summary>
