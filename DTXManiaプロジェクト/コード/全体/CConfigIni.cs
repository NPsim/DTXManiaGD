...
namespace DTXMania
{
	internal class CConfigIni
	{
		...
		public bool bDiscordPresence; // 2019.12.25 sub; Add Discord Rich Presence
		public bool bSTAGEFAILED有効;
		public STDGBVALUE<bool> bSudden;
		public bool bTight;
		public STDGBVALUE<bool> bGraph有効;     // #24074 2011.01.23 add ikanick
		public bool bShowPerfectRegion;     // 2019.12.25 sub; Perfect Hit Region
        ...
        */
        public void SwapGuitarBassInfos_AutoFlags()
        {
            ...
            for (int i = 0; i < looptime; i++)							// こんなに離れたところを独立して修正しなければならない設計ではいけませんね_KMD__KMD__KMD_
            {
...
//          this.nASIOBufferSizeMs = 0;                 // #24820 2012.12.25 yyagi 初期値は0(自動設定)
	...
	//------------------------------
			...
			sw.WriteLine();
            sw.WriteLine( "; Discord Rich Presence Synchronization (0:OFF, 1:ON)" );
			sw.WriteLine( "DiscordRPC={0}", this.bDiscordPresence ? 1 : 0 );
			sw.WriteLine();
            #region [ GDオプション ]
...
//			sw.WriteLine();																	//
			...
			sw.WriteLine();
			sw.WriteLine( "; 画像やサウンドの作成_KMD_解放に関するLog出力(0:OFF, 1:ON)" );
			...
			sw.WriteLine();
			sw.WriteLine( "; Show Perfect Hit Region" );
			sw.WriteLine( "ShowPerfect={0}", this.bShowPerfectRegion ? 1 : 0);
			sw.WriteLine();
								...
								switch( unknown )
								{
									...
									case Eセクション種別.System:
										...
										//----------------------------------------
											...
											else if (str3.Equals("DiscordRPC"))
											{
												this.bDiscordPresence = C変換.bONorOFF(str4[0]);
											}
									...
									case Eセクション種別.HitRange:
										...
										else if( str3.Equals( "ShowPerfect" ) )
										{
											this.bShowPerfectRegion = C変換.bONorOFF(str4[0]);
										}
										continue;
									//-----------------------------
									#endregion
