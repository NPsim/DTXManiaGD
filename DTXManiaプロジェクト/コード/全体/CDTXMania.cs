...
namespace DTXMania
{
    internal class CDTXMania : Game
    {
        ...
        public static readonly string VERSION = "Ver3.62GD(191226) DRPC"; // 2019.12.16 sub; checked-out from DTXManiaXG verK REVISION-575 (Ver3.62GD(190426))
        ...
        public bool b次のタイミングで全画面_KMD_ウィンドウ切り替えを行う
        {
            get;
            set;
        }
        ...
        public void t全画面_KMD_ウィンドウモード切り替え()
        {
        ...
        {
            if (CDTXMania.ConfigIni.bDiscordPresence && Discord.Initialized)
            {
                Discord.client.Invoke(); // 2019.12.16 sub; Add Discord RPC - Required for Activity updates
            }
        }
            ...
            if (r現在のステージ != null)
            {
                ...
                switch (r現在のステージ.eステージID)
                {
                    ...
                    case CStage.Eステージ.曲読み込み:
                        if (EnumSongs != null)
                        {
                            #region [ (特定条件時) 曲検索スレッドの起動_KMD_開始 ]
                                 ...
                                 !EnumSongs.IsSongListEnumStarted)
                            {
                                ...
                                EnumSongs.StartEnumFromDisk();		// 曲検索スレッドの起動_KMD_開始
...
for (int i = 0; i < 3; i++) {
	stage演奏ギター画面.fDamageLevelFactor[i] = ConfigIni.fDamageLevelFactor[i];
...
#endif
            #region [ 全画面_KMD_ウインドウ切り替え ]
            if (this.b次のタイミングで全画面_KMD_ウィンドウ切り替えを行う)
            {
                ConfigIni.b全画面モード = !ConfigIni.b全画面モード;
                app.t全画面_KMD_ウィンドウモード切り替え();
                this.b次のタイミングで全画面_KMD_ウィンドウ切り替えを行う = false;
            }
            #endregion
            #region [ 垂直基線同期切り替え ]
        ...
        private void t起動処理()
        {
...
#if WindowedFullscreen
			this.t全画面_KMD_ウィンドウモード切り替え();				// #30666 2013.2.2 yyagi: finalize settings for "Maximized window mode"
#endif
            ...
            //---------------------
            #endregion
            #region [ Discord RPC Initialization ]
            // 2019.12.16 sub; Add Discord RPC - Initialize RPC Instance
            //---------------------
            if (CDTXMania.ConfigIni.bDiscordPresence)
            {
                try
                {
                    Discord.Initialize();
                }
                catch (Exception) { } // Discord can't connect; try running DTXMania as Administrator
            }
            //---------------------
            #endregion
        ...
        private void t終了処理()
        {
            if (!this.b終了処理完了済み)
            {
                ...
                //---------------------
                #endregion
                #region [ Discord RPC Termination ]
                // 2019.12.16 sub; Add Discord RPC - Dispose RPC instance
                //---------------------
                if (CDTXMania.ConfigIni.bDiscordPresence && Discord.Initialized)
                {
