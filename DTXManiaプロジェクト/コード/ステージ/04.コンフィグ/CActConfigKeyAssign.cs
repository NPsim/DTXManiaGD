...
namespace DTXMania
{
	internal class CActConfigKeyAssign : CActivity
	{
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
				if( this.bキー入力待ち )
				{
					...
					else if( ( this.tキーチェックとアサイン_KMD_Keyboard() || this.tキーチェックとアサイン_KMD_MidiIn() ) || ( this.tキーチェックとアサイン_KMD_Joypad() || this.tキーチェックとアサイン_KMD_Mouse() ) )
					{
						this.bキー入力待ち = false;
						CDTXMania.Input管理.tポーリング( CDTXMania.app.bApplicationActive, false );
					}
				...
				for( int i = 0; i < 0x10; i++ )
				{
					...
					{
						case E入力デバイス.キーボード:
							this.tアサインコードの描画_KMD_Keyboard( i + 1, x + 20, y, stkeyassignArray[ i ].ID, stkeyassignArray[ i ].コード, this.n現在の選択行 == i );
							break;

						case E入力デバイス.MIDI入力:
							this.tアサインコードの描画_KMD_MidiIn( i + 1, x + 20, y, stkeyassignArray[ i ].ID, stkeyassignArray[ i ].コード, this.n現在の選択行 == i );
							break;

						case E入力デバイス.ジョイパッド:
							this.tアサインコードの描画_KMD_Joypad( i + 1, x + 20, y, stkeyassignArray[ i ].ID, stkeyassignArray[ i ].コード, this.n現在の選択行 == i );
							break;

						case E入力デバイス.マウス:
							this.tアサインコードの描画_KMD_Mouse( i + 1, x + 20, y, stkeyassignArray[ i ].ID, stkeyassignArray[ i ].コード, this.n現在の選択行 == i );
							break;
		...
		private void tアサインコードの描画_KMD_Joypad( int line, int x, int y, int nID, int nCode, bool b強調 )
		{
			string str = "";
			switch( nCode )
			{
		...
		private void tアサインコードの描画_KMD_Keyboard( int line, int x, int y, int nID, int nCode, bool b強調 )
		{
			string str = null;
		...
		private void tアサインコードの描画_KMD_MidiIn( int line, int x, int y, int nID, int nCode, bool b強調 )
		{
			CDTXMania.stageコンフィグ.actFont.t文字列描画( x, y, string.Format( "{0,2}. MidiIn #{1} code.{2}", line, nID, nCode ), b強調, 0.75f );
		}
		private void tアサインコードの描画_KMD_Mouse( int line, int x, int y, int nID, int nCode, bool b強調 )
		{
			CDTXMania.stageコンフィグ.actFont.t文字列描画( x, y, string.Format( "{0,2}. Mouse Button{1}", line, nCode ), b強調, 0.75f );
		}
		private bool tキーチェックとアサイン_KMD_Joypad()
		{
		...
		private bool tキーチェックとアサイン_KMD_Keyboard()
		{
		...
		private bool tキーチェックとアサイン_KMD_MidiIn()
		{
			foreach( IInputDevice device in CDTXMania.Input管理.list入力デバイス )
			{
		...
		private bool tキーチェックとアサイン_KMD_Mouse()
		{
			for( int i = 0; i < 8; i++ )
			{
