    ...
    internal class CStageコンフィグ : CStage
    {
        ...
        private void tカーソルを下へ移動する()
        {
            ...
            else
            {
                ...
                {
                    case 0:
                        this.actList.t項目リストの設定_KMD_System();
                        break;

                    //case 1:
                    //    this.actList.t項目リストの設定_KMD_KeyAssignDrums();
                    //    break;

                    //case 2:
                    //    this.actList.t項目リストの設定_KMD_KeyAssignGuitar();
                    //    break;

                    //case 3:
                    //    this.actList.t項目リストの設定_KMD_KeyAssignBass();
                    //    break;

                    case 1:
                        this.actList.t項目リストの設定_KMD_Drums();
                        break;

                    case 2:
                        this.actList.t項目リストの設定_KMD_Guitar();
                        break;

                    case 3:
                        this.actList.t項目リストの設定_KMD_Bass();
                        break;

                    case 4:
                        this.actList.t項目リストの設定_KMD_Exit();
                        break;
                }
                this.t説明文パネルに現在選択されているメニューの説明を描画する();
            }
		...
		private void t説明文パネルに現在選択されている項目の説明を描画する()
		{
			try
			{
				var image = new Bitmap( (int)(400), (int)(192) );		// 説明文領域サイズの縦横 2 倍。（描画時に 0.5 倍で表示する_KMD__KMD__KMD_のは中止。処理速度向上のため。）
				var graphics = Graphics.FromImage( image );
				graphics.TextRenderingHint = System.Drawing.Text.TextRenderingHint.AntiAlias;
