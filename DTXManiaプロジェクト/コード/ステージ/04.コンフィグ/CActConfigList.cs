...
namespace DTXMania
{
    internal class CActConfigList : CActivity
    {
        ...
        // メソッド
        #region [ t項目リストの設定_KMD_System() ]
        public void t項目リストの設定_KMD_System()
        {
            ...
            */
            ...
            // 2019.12.25 sub; Add Discord Rich Presence
            this.iDiscordPresence = new CItemToggle("Sync Discord", CDTXMania.ConfigIni.bDiscordPresence,
                "ディスコード同期\n",
                "Discord Rich Presence Synchronization\n\n" +
                "Show InMenu/InGame (Current Song, Artist, Difficulty, Level, Time Remaining)\n\n" +
                "Requires restart");
            this.list項目リスト.Add(this.iDiscordPresence);
        ...
        #endregion
        #region [ t項目リストの設定_KMD_Drums() ]
        public void t項目リストの設定_KMD_Drums()
        {
            this.tConfigIniへ記録する();
            this.list項目リスト.Clear();
            ...
            // 2019.12.25 sub Perfect Hit Zone
            this.iDrumsShowPerfectRegion = new CItemToggle("Show Perfect Region", CDTXMania.ConfigIni.bShowPerfectRegion,
                "PERFECTリージョンを表示。\n",
                "Show Perfect hit region.");
            this.list項目リスト.Add(this.iDrumsShowPerfectRegion);
        ...
        #endregion
        #region [ t項目リストの設定_KMD_Guitar() ]
        public void t項目リストの設定_KMD_Guitar()
        {
            this.tConfigIniへ記録する();
            this.list項目リスト.Clear();
        ...
        #endregion
        #region [ t項目リストの設定_KMD_Bass() ]
        public void t項目リストの設定_KMD_Bass()
        {
            this.tConfigIniへ記録する();
            this.list項目リスト.Clear();
        ...
        public void tEnter押下()
        {
            ...
            else
            {
                ...
                {
                    CDTXMania.app.b次のタイミングで全画面_KMD_ウィンドウ切り替えを行う = true;
                }
        ...
        public void t項目リストの設定_KMD_Exit()
        {
            this.tConfigIniへ記録する();
            this.eメニュー種別 = Eメニュー種別.Unknown;
        }
        public void t項目リストの設定_KMD_KeyAssignSystem()
        {
            //this.tConfigIniへ記録する();
            this.list項目リスト.Clear();
        ...
        public void t項目リストの設定_KMD_KeyAssignDrums()
        {
            //			this.tConfigIniへ記録する();
            this.list項目リスト.Clear();
        ...
        public void t項目リストの設定_KMD_KeyAssignGuitar()
        {
            //			this.tConfigIniへ記録する();
            this.list項目リスト.Clear();
        ...
        public void t項目リストの設定_KMD_KeyAssignBass()
        {
            //			this.tConfigIniへ記録する();
            this.list項目リスト.Clear();
        ...
        public override void On活性化()
        {
            ...
            this.t項目リストの設定_KMD_Bass();		// #27795 2012.3.11 yyagi; System設定の中でDrumsの設定を参照しているため、
            this.t項目リストの設定_KMD_Guitar();	// 活性化の時点でDrumsの設定も入れ込んでおかないと、System設定中に例外発生することがある。
            this.t項目リストの設定_KMD_Drums();	// 
            this.t項目リストの設定_KMD_System();	// 順番として、最後にSystemを持ってくること。設定一覧の初期位置がSystemのため。
            this.b要素値にフォーカス中 = false;
...
//					CDTXMania.tテクスチャの解放( ref ctItem );
				...
				switch( this.list項目リスト[ nItem ].e種別 )
				{
					...
					case CItemBase.E種別.リスト:	// #28195 2012.5.2 yyagi: add Skin supports
						...
						//-----------------
						{
							...
							#region [ 必要な場合に、Skinのサンプルを生成_KMD_描画する。#28195 2012.5.2 yyagi ]
        ...
        private CItemToggle iDiscordPresence; // 2019.12.25 sub; Add Discord Rich Presence
        ...
        private CItemToggle iDrumsShowPerfectRegion;
        ...
        private void tConfigIniへ記録する()
        {
            ...
            {
                case Eメニュー種別.System:
                    this.tConfigIniへ記録する_KMD_System();
                    this.tConfigIniへ記録する_KMD_KeyAssignSystem();
                    return;

                case Eメニュー種別.Drums:
                    this.tConfigIniへ記録する_KMD_Drums();
                    this.tConfigIniへ記録する_KMD_KeyAssignDrums();
                    return;

                case Eメニュー種別.Guitar:
                    this.tConfigIniへ記録する_KMD_Guitar();
                    this.tConfigIniへ記録する_KMD_KeyAssignGuitar();
                    return;

                case Eメニュー種別.Bass:
                    this.tConfigIniへ記録する_KMD_Bass();
                    this.tConfigIniへ記録する_KMD_KeyAssignBass();
                    return;
            }
        }
        private void tConfigIniへ記録する_KMD_KeyAssignBass()
        {
        }
        private void tConfigIniへ記録する_KMD_KeyAssignDrums()
        {
        }
        private void tConfigIniへ記録する_KMD_KeyAssignGuitar()
        {
        }
        private void tConfigIniへ記録する_KMD_KeyAssignSystem()
        {
        }
        private void tConfigIniへ記録する_KMD_System()
        {
            ...
            CDTXMania.ConfigIni.bDiscordPresence = this.iDiscordPresence.bON; // 2019.12.25 sub; Add Discord Rich Presence
        ...
        private void tConfigIniへ記録する_KMD_Bass()
        {
        ...
        private void tConfigIniへ記録する_KMD_Drums()
        {
            ...
            CDTXMania.ConfigIni.bShowPerfectRegion = this.iDrumsShowPerfectRegion.bON;

            //CDTXMania.ConfigIni.eDark = (Eダークモード) this.iCommonDark.n現在選択されている項目番号;     // ダークはプリセット切り替えとして使うため、保存はしない。
        }
        private void tConfigIniへ記録する_KMD_Guitar()
        {
