...
namespace DTXMania
{
	internal class CStage選曲 : CStage
	{
        ...
        /// <para>r現在演奏中の曲のスコア の読み込み時に、自動検索_KMD_抽出_KMD_生成される。</para>
        /// </summary>
...
//						}
				...
				{
					CDTXMania.Skin.bgm選曲画面.n音量_KMD_次に鳴るサウンド = 100;
...
//		private CActオプションパネル actオプションパネル;
		...
		public class CCommandHistory		// #24063 2011.1.16 yyagi コマンド入力履歴を保持_KMD_確認するクラス
		{
...
//Debug.WriteLine( "" );
		...
		private void t曲をランダム選択する()
		{
			...
			if( ( song.stackランダム演奏番号.Count == 0 ) || ( song.listランダム用ノードリスト == null ) )
			{
				...
				{
					song.listランダム用ノードリスト = this.t指定された曲が存在する場所の曲を列挙する_KMD_子リスト含む( song );
				}
		...
		private List<C曲リストノード> t指定された曲が存在する場所の曲を列挙する_KMD_子リスト含む( C曲リストノード song )
		{
			...
			if( ( song == null ) && ( CDTXMania.Songs管理.list曲ルート.Count > 0 ) )
			{
				foreach( C曲リストノード c曲リストノード in CDTXMania.Songs管理.list曲ルート )
				{
					...
					{
						this.t指定された曲の子リストの曲を列挙する_KMD_孫リスト含む( c曲リストノード, ref list );
					}
				}
				return list;
			}
			this.t指定された曲の子リストの曲を列挙する_KMD_孫リスト含む( song, ref list );
			return list;
		}
		private void t指定された曲の子リストの曲を列挙する_KMD_孫リスト含む( C曲リストノード r親, ref List<C曲リストノード> list )
		{
