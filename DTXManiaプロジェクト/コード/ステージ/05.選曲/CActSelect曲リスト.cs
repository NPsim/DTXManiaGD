...
//					int x = this.ptバーの基本座標[ n見た目の行番号 ].X + ( (int) ( ( this.ptバーの基本座標[ n次のパネル番号 ].X - this.ptバーの基本座標[ n見た目の行番号 ].X ) * ( ( (double) Math.Abs( this.n現在のスクロールカウンタ ) ) / 100.0 ) ) );
		...
		private void tスキル値の描画( int x, int y, int nスキル値 )
		{
			...
			if( n百の位 > 0 )
				this.tスキル値の描画_KMD_１桁描画( x, y, n百の位, color );


			// 十の位の描画。

			if( n百の位 != 0 || n十の位 != 0 )
				this.tスキル値の描画_KMD_１桁描画( x + 14, y, n十の位, color );


			// 一の位の描画。

			this.tスキル値の描画_KMD_１桁描画( x + 0x1c, y, n一の位, color );
		}
		private void tスキル値の描画_KMD_１桁描画( int x, int y, int n数値, int color )
		{
			int dx = ( n数値 % 5 ) * 9;
			int dy = ( n数値 / 5 ) * 12;
        ...
        private void tアイテム数の描画()
        {
            ...
            {
                tアイテム数の描画_KMD_１桁描画(x, y, s[p]);
                x -= 16;
            }
        }
        private void tアイテム数の描画_KMD_１桁描画(int x, int y, char s数値)
        {
            int dx, dy;
            if (s数値 == '/')
            {
