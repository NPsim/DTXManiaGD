...
namespace DTXMania
{
	internal class CActSelectPresound : CActivity
	{
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
				if( ( this.ctBGMフェードイン用 != null ) && this.ctBGMフェードイン用.b進行中 )
				{
					...
					CDTXMania.Skin.bgm選曲画面.n音量_KMD_現在のサウンド = this.ctBGMフェードイン用.n現在の値;
				...
				if( ( this.ctBGMフェードアウト用 != null ) && this.ctBGMフェードアウト用.b進行中 )
				{
					...
					CDTXMania.Skin.bgm選曲画面.n音量_KMD_現在のサウンド = 100 - this.ctBGMフェードアウト用.n現在の値;
					if( this.ctBGMフェードアウト用.b終了値に達した )
					{
						this.ctBGMフェードアウト用.t停止();
					}
				}
				this.t進行処理_KMD_プレビューサウンド();
			}
			return 0;
		}
		...
		private void tBGMフェードアウト開始()
		{
			...
			this.ctBGMフェードアウト用.n現在の値 = 100 - CDTXMania.Skin.bgm選曲画面.n音量_KMD_現在のサウンド;
		}
		private void tBGMフェードイン開始()
		{
			...
			this.ctBGMフェードイン用.n現在の値 = CDTXMania.Skin.bgm選曲画面.n音量_KMD_現在のサウンド;
		}
		...
		private void t進行処理_KMD_プレビューサウンド()
		{
