	...
	internal class CStage結果 : CStage
	{
        ...
        private void SaveGhost(int sectionIndex)
        {
            ...
            //if(false)
            {
                ...
                //        sw.WriteLine( "Perfect=" + cScoreData.nPerfect数_KMD_Auto含まない );
                //        sw.WriteLine( "Great=" + cScoreData.nGreat数_KMD_Auto含まない );
                //        sw.WriteLine( "Good=" + cScoreData.nGood数_KMD_Auto含まない );
                //        sw.WriteLine( "Poor=" + cScoreData.nPoor数_KMD_Auto含まない );
                //        sw.WriteLine( "Miss=" + cScoreData.nMiss数_KMD_Auto含まない );
								...
								foreach( STInputEvent event2 in events )
								{
									...
									if( ( ( rChip != null ) && ( rChip.nチャンネル番号 >= 0x11 ) ) && ( rChip.nチャンネル番号 <= 0x1b ) )
									{
										...
										if( ( nLane == 1 ) && ( ( rChip.nチャンネル番号 == 0x11 ) || ( ( rChip.nチャンネル番号 == 0x18 ) && ( this.n最後に再生したHHのチャンネル番号 != 0x18 ) ) ) )
										{
											...
											this.n最後に再生したHHのWAV番号 = rChip.n整数値_KMD_内部番号;
