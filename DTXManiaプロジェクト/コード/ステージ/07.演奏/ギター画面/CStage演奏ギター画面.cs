		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
				...
				if( base.b初めての進行描画 )
				{
                    ...
                    this.UnitTime = ((60.0 / (CDTXMania.stage演奏ギター画面.actPlayInfo.dbBPM) / 14.0)); //2014.01.14.kairera0467 これも動かしたいのだが_KMD__KMD__KMD__KMD_
...
//				    this.t進行描画_KMD_パネル文字列();
				...
				this.t進行描画_KMD_レーンフラッシュGB();

				this.t進行描画_KMD_DANGER();

				this.t進行描画_KMD_WailingBonus();
				this.t進行描画_KMD_譜面スクロール速度();
				this.t進行描画_KMD_チップアニメ();
                this.t進行描画_KMD_小節線(E楽器パート.GUITAR);
                flag = this.t進行描画_KMD_チップ(E楽器パート.GUITAR);
                this.t進行描画_KMD_RGBボタン();
                this.t進行描画_KMD_ギターベース判定ライン();
				this.t進行描画_KMD_判定文字列();
                this.t進行描画_KMD_ゲージ();
                ...
                this.t進行描画_KMD_コンボ();
				this.t進行描画_KMD_演奏情報();
				//this.t進行描画_KMD_Wailing枠();

                this.t進行描画_KMD_チップファイアGB();
				this.t進行描画_KMD_STAGEFAILED();
                flag2 = this.t進行描画_KMD_フェードイン_KMD_アウト();
		...
		protected override E判定 tチップのヒット処理( long nHitTime, CDTX.CChip pChip, bool bCorrectLane )
		{
            ...
            if( pChip.e楽器パート == E楽器パート.GUITAR && CDTXMania.ConfigIni.bGraph有効.Guitar )
            {
                if( CDTXMania.ConfigIni.nSkillMode == 0 )
			        this.actGraph.dbグラフ値現在_渡 = CScoreIni.t旧演奏型スキルを計算して返す( CDTXMania.DTX.n可視チップ数.Guitar, this.nヒット数_KMD_Auto含まない.Guitar.Perfect, this.nヒット数_KMD_Auto含まない.Guitar.Great, this.nヒット数_KMD_Auto含まない.Guitar.Good, this.nヒット数_KMD_Auto含まない.Guitar.Poor, this.nヒット数_KMD_Auto含まない.Guitar.Miss, E楽器パート.GUITAR,  bIsAutoPlay );
                else
				    ...
				    CDTXMania.DTX.n可視チップ数.Guitar > 0 )
    			{
	    			...
	    			this.actGraph.dbグラフ値現在_渡 = 100 *
		    						(this.nヒット数_KMD_Auto含まない.Guitar.Perfect * 17 +
			    					 this.nヒット数_KMD_Auto含まない.Guitar.Great * 7 +
				    				 this.actCombo.n現在のコンボ数.最高値.Guitar * 3) / (20.0 * CDTXMania.DTX.n可視チップ数.Guitar );
    			}

                this.actGraph.n現在のAutoを含まない判定数_渡[ 0 ] = this.nヒット数_KMD_Auto含まない.Guitar.Perfect;
                this.actGraph.n現在のAutoを含まない判定数_渡[ 1 ] = this.nヒット数_KMD_Auto含まない.Guitar.Great;
                this.actGraph.n現在のAutoを含まない判定数_渡[ 2 ] = this.nヒット数_KMD_Auto含まない.Guitar.Good;
                this.actGraph.n現在のAutoを含まない判定数_渡[ 3 ] = this.nヒット数_KMD_Auto含まない.Guitar.Poor;
                this.actGraph.n現在のAutoを含まない判定数_渡[ 4 ] = this.nヒット数_KMD_Auto含まない.Guitar.Miss;
            }
            else if( pChip.e楽器パート == E楽器パート.BASS && CDTXMania.ConfigIni.bGraph有効.Bass )
            {
                if( CDTXMania.ConfigIni.nSkillMode == 0 )
			        this.actGraph.dbグラフ値現在_渡 = CScoreIni.t旧演奏型スキルを計算して返す( CDTXMania.DTX.n可視チップ数.Bass, this.nヒット数_KMD_Auto含まない.Bass.Perfect, this.nヒット数_KMD_Auto含まない.Bass.Great, this.nヒット数_KMD_Auto含まない.Bass.Good, this.nヒット数_KMD_Auto含まない.Bass.Poor, this.nヒット数_KMD_Auto含まない.Bass.Miss, E楽器パート.BASS,  bIsAutoPlay );
                else
	    		    this.actGraph.dbグラフ値現在_渡 = CScoreIni.t演奏型スキルを計算して返す( CDTXMania.DTX.n可視チップ数.Bass, this.nヒット数_KMD_Auto含まない.Bass.Perfect, this.nヒット数_KMD_Auto含まない.Bass.Great, this.nヒット数_KMD_Auto含まない.Bass.Good, this.nヒット数_KMD_Auto含まない.Bass.Poor, this.nヒット数_KMD_Auto含まない.Bass.Miss, this.actCombo.n現在のコンボ数.最高値.Bass, E楽器パート.BASS,  bIsAutoPlay );
				    ...
				    CDTXMania.DTX.n可視チップ数.Bass > 0 )
    			{
	    			...
	    			this.actGraph.dbグラフ値現在_渡 = 100 *
		    						(this.nヒット数_KMD_Auto含まない.Bass.Perfect * 17 +
			    					 this.nヒット数_KMD_Auto含まない.Bass.Great * 7 +
				    				 this.actCombo.n現在のコンボ数.最高値.Bass * 3) / (20.0 * CDTXMania.DTX.n可視チップ数.Bass );
    			}

                this.actGraph.n現在のAutoを含まない判定数_渡[ 0 ] = this.nヒット数_KMD_Auto含まない.Bass.Perfect;
                this.actGraph.n現在のAutoを含まない判定数_渡[ 1 ] = this.nヒット数_KMD_Auto含まない.Bass.Great;
                this.actGraph.n現在のAutoを含まない判定数_渡[ 2 ] = this.nヒット数_KMD_Auto含まない.Bass.Good;
                this.actGraph.n現在のAutoを含まない判定数_渡[ 3 ] = this.nヒット数_KMD_Auto含まない.Bass.Poor;
                this.actGraph.n現在のAutoを含まない判定数_渡[ 4 ] = this.nヒット数_KMD_Auto含まない.Bass.Miss;
            }
			return eJudgeResult;
		}
		protected override void tチップのヒット処理_KMD_BadならびにTight時のMiss( E楽器パート part )
		{
			this.tチップのヒット処理_KMD_BadならびにTight時のMiss( part, 0, E楽器パート.GUITAR );
		}
		protected override void tチップのヒット処理_KMD_BadならびにTight時のMiss( E楽器パート part, int nLane )
		{
			this.tチップのヒット処理_KMD_BadならびにTight時のMiss( part, nLane, E楽器パート.GUITAR );
		}
		...
		{
		    base.t進行描画_KMD_BGA( 500, 50 );
		}
         */
		protected override void t進行描画_KMD_DANGER()			// #23631 2011.4.19 yyagi
		{
		...
		{
			base.t進行描画_KMD_Wailing枠( 292, 0x251,
				CDTXMania.ConfigIni.bReverse.Guitar ? 340 : 130,
				CDTXMania.ConfigIni.bReverse.Bass ?   340 : 130
			);
		}
		private void t進行描画_KMD_ギターベース判定ライン()	// yyagi: ドラム画面とは座標が違うだけですが、まとめづらかったのでそのまま放置してます。
		{
		...
		{
			base.t進行描画_KMD_パネル文字列( 0xb5, 430 );
		}
         */

		protected override void t進行描画_KMD_演奏情報()
		{
			base.t進行描画_KMD_演奏情報( 500, 257 );
		}
...
#if false
		protected override void t進行描画_KMD_チップ_KMD_ベース( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip )
		{
			if ( configIni.bGuitar有効 )
			{
