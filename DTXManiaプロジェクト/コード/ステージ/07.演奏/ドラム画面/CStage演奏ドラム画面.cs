...
//          base.list子Activities.Add( this.actStageCleared = new CAct演奏ステージクリア());
		...
		{
			base.t演奏結果を格納する_KMD_ドラム( out Drums );
			base.t演奏結果を格納する_KMD_ギター( out Guitar );
			base.t演奏結果を格納する_KMD_ベース( out Bass );
			...
			for ( int i = 0; i < 12; i++ )
			{
				...
				{
					r空打ちドラムチップ[ i ] = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮( CDTXMania.Timer.n現在時刻, this.nパッド0Atoチャンネル0A[ i ], this.nInputAdjustTimeMs.Drums );
				}
			}
		}
		...
		public override void OnManagedリソースの作成()
		{
			if( !base.b活性化してない )
			{
                ...
                this.txPerfectZone = CDTXMania.tテクスチャの生成(CSkin.Path(@"Graphics\ScreenPlayDrums perfectzone.png"));
		...
		public override void OnManagedリソースの解放()
		{
			if( !base.b活性化してない )
			{
				...
				CDTXMania.tテクスチャの解放( ref this.txPerfectZone );
...
//					if ( this.bDTXVmode )
                    {
                        ...
                        #region [ BGMの途中再生開始 (CDTXのt入力_KMD_行解析_KMD_チップ配置()で小節番号が+1されているのを削っておくこと) ]
...
// 未実装 ここまで
                ...
                this.t進行描画_KMD_背景();
                this.t進行描画_KMD_MIDIBGM();
                this.t進行描画_KMD_AVI();
                this.t進行描画_KMD_レーンフラッシュD();
                this.t進行描画_KMD_譜面スクロール速度();
                this.tDrawPerfectZone();
                this.t進行描画_KMD_チップアニメ();
                this.t進行描画_KMD_小節線( E楽器パート.DRUMS );
                this.t進行描画_KMD_チップ_KMD_模様のみ( E楽器パート.DRUMS );
                bIsFinishedPlaying = this.t進行描画_KMD_チップ( E楽器パート.DRUMS );
                #region[ シャッター ]
                ...
                #endregion
                this.t進行描画_KMD_判定ライン();
                this.t進行描画_KMD_ドラムパッド();
                bIsFinishedFadeout = this.t進行描画_KMD_フェードイン_KMD_アウト();
                if (bIsFinishedPlaying && (base.eフェーズID == CStage.Eフェーズ.共通_通常状態) )
                {
                    ...
                    else
                    {
                        ...
                        if (base.nヒット数_KMD_Auto含まない.Drums.Miss + base.nヒット数_KMD_Auto含まない.Drums.Poor == 0)
                        {
                            this.nパフェ数 = CDTXMania.ConfigIni.bドラムが全部オートプレイである ? this.nパフェ数 = base.nヒット数_KMD_Auto含む.Drums.Perfect : base.nヒット数_KMD_Auto含まない.Drums.Perfect;
...
//                  this.t進行描画_KMD_パネル文字列();
                if (CDTXMania.ConfigIni.nInfoType == 1)
                    this.t進行描画_KMD_ステータスパネル();
                this.t進行描画_KMD_ゲージ();
                this.t進行描画_KMD_コンボ();
                this.t進行描画_KMD_グラフ();
                this.t進行描画_KMD_演奏情報();
                this.t進行描画_KMD_判定文字列1_KMD_通常位置指定の場合();
                this.t進行描画_KMD_判定文字列2_KMD_判定ライン上指定の場合();
                this.t進行描画_KMD_チップファイアD();
                this.t進行描画_KMD_STAGEFAILED();
                bすべてのチップが判定された = true;
                if (bIsFinishedFadeout)
                {
                    if (!CDTXMania.Skin.soundステージクリア音.b再生中 && !CDTXMania.Skin.soundSTAGEFAILED音.b再生中)
                    {
                        ...
                        this.nミス数 = base.nヒット数_KMD_Auto含まない.Drums.Miss + base.nヒット数_KMD_Auto含まない.Drums.Poor;
                        switch (nミス数)
                        {
                            case 0:
                                {
                                    this.nパフェ数 = base.nヒット数_KMD_Auto含まない.Drums.Perfect;
                                    if (CDTXMania.ConfigIni.bドラムが全部オートプレイである)
                                    {
                                        this.nパフェ数 = base.nヒット数_KMD_Auto含む.Drums.Perfect;
                                    }
                                    if (nパフェ数 == CDTXMania.DTX.n可視チップ数.Drums)
                                    #region[ エクセ ]
                                    {
...
//		private CTexture txレーンフレームGB;
		...
		protected override E判定 tチップのヒット処理( long nHitTime, CDTX.CChip pChip, bool bCorrectLane )
		{
            ...
            {
                this.actGraph.dbグラフ値現在_渡 = CScoreIni.t旧演奏型スキルを計算して返す(CDTXMania.DTX.n可視チップ数.Drums, this.nヒット数_KMD_Auto含まない.Drums.Perfect, this.nヒット数_KMD_Auto含まない.Drums.Great, this.nヒット数_KMD_Auto含まない.Drums.Good, this.nヒット数_KMD_Auto含まない.Drums.Poor, this.nヒット数_KMD_Auto含まない.Drums.Miss, E楽器パート.DRUMS, bIsAutoPlay);
            }
				...
				CDTXMania.DTX.n可視チップ数.Drums > 0 )
			{
				...
				this.actGraph.dbグラフ値現在_渡 = 100 *
								( this.nヒット数_KMD_Auto含まない.Drums.Perfect * 17 +
								 this.nヒット数_KMD_Auto含まない.Drums.Great * 7 +
		...
		protected override void tチップのヒット処理_KMD_BadならびにTight時のMiss( E楽器パート part )
		{
			this.tチップのヒット処理_KMD_BadならびにTight時のMiss( part, 0, E楽器パート.DRUMS );
		}
		protected override void tチップのヒット処理_KMD_BadならびにTight時のMiss( E楽器パート part, int nLane )
		{
			this.tチップのヒット処理_KMD_BadならびにTight時のMiss( part, nLane, E楽器パート.DRUMS );
		}
		...
		private bool tドラムヒット処理( long nHitTime, Eパッド type, CDTX.CChip pChip, int n強弱度合い0to127 )
		{
			...
			if( CDTXMania.ConfigIni.bドラム打音を発声する )
			{
				...
				else
				{
					...
					rChip = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮( nHitTime, this.nパッド0Atoチャンネル0A[ (int) hH ], nInputAdjustTime );
					if( rChip == null )
					{
						rChip = pChip;
					}
        ...
        /*
		protected override void t進行描画_KMD_AVI()
		{
			base.t進行描画_KMD_AVI( 0, 0 );
		}
		protected override void t進行描画_KMD_BGA()
		{
			base.t進行描画_KMD_BGA( 990, 0 );
		}
         */
		protected override void t進行描画_KMD_DANGER()
		{
			this.actDANGER.t進行描画( this.actGauge.IsDanger(E楽器パート.DRUMS), false, false );
		}

		protected override void t進行描画_KMD_Wailing枠()
		{
			base.t進行描画_KMD_Wailing枠( 587, 478,
        ...
        /*
		private void t進行描画_KMD_ギターベースフレーム()
		{
		...
		private void t進行描画_KMD_ギターベース判定ライン()		// yyagi: ギタレボモードとは座標が違うだけですが、まとめづらかったのでそのまま放置してます。
		{
		...
		private void t進行描画_KMD_チップファイアD()
        {
			this.actChipFireD.On進行描画();
        }
        private void t進行描画_KMD_ドラムパッド()
        {
            this.actPad.On進行描画();
        }

        /*
        protected override void t進行描画_KMD_パネル文字列()
        {
            base.t進行描画_KMD_パネル文字列(912, 640);
        }
         */

		protected override void t進行描画_KMD_演奏情報()
		{
			base.t進行描画_KMD_演奏情報( 1000, 257 );
		}

		protected override void t入力処理_KMD_ドラム()
        {
            ...
            for (int nPad = 0; nPad < (int)Eパッド.MAX; nPad++)
            {
                ...
                foreach (STInputEvent inputEvent in listInputEvent)
                {
                        ...
                        else
                        {
                            ...
                            switch (((Eパッド)nPad))
                            {
                                case Eパッド.HH:
                                    ...
                                    //-----------------------------
                                    {
                                        CDTX.CChip chipHC = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[0], nInputAdjustTime);
                                        CDTX.CChip chipHO = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[7], nInputAdjustTime);
                                        CDTX.CChip chipLC = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[9], nInputAdjustTime);
                                ...
                                case Eパッド.LT:
                                    ...
                                    //-----------------------------
                                    {
                                        CDTX.CChip chipLT = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[4], nInputAdjustTime);
                                        CDTX.CChip chipFT = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[5], nInputAdjustTime);
                                ...
                                case Eパッド.CY:
                                    ...
                                    //-----------------------------
                                    {
                                        ...
                                        CDTX.CChip chipCY = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[6], nInputAdjustTime);
                                        CDTX.CChip chipRD = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[8], nInputAdjustTime);
                                ...
                                case Eパッド.BD:
                                    ...
                                    //-----------------------------
                                    {
                                        CDTX.CChip chipBD = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[2], nInputAdjustTime + nPedalLagTime);
                                        CDTX.CChip chipLP = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[10], nInputAdjustTime + nPedalLagTime);
                                        CDTX.CChip chipLBD = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[11], nInputAdjustTime + nPedalLagTime);
                                ...
                                default:
                                    #region [ *** ]
                                    //-----------------------------
                                    rChip = this.r指定時刻に一番近いChip_KMD_ヒット未済問わず不可視考慮(nTime, this.nパッド0Atoチャンネル0A[nPad], nInputAdjustTime);
                                    //-----------------------------
                                    #endregion
                                    break;
                            }
                    ...
                    if (CDTXMania.ConfigIni.bTight)
                        this.tチップのヒット処理_KMD_BadならびにTight時のMiss(E楽器パート.DRUMS, this.nパッド0Atoレーン07[nPad]);
                    //-----------------------------
                    #endregion
                }
            }
        ...
        protected override void t進行描画_KMD_チップ_KMD_模様のみ_KMD_ドラムス(CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip)
        {
		...
		protected override void t進行描画_KMD_チップ_KMD_ドラムス( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip )
		{
			if( configIni.bDrums有効 )
			{
				#region [ Sudden処理 ]
        ...
        protected override void t進行描画_KMD_チップ_KMD_ギターベース(CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, E楽器パート inst)
		{
			base.t進行描画_KMD_チップ_KMD_ギターベース( configIni, ref dTX, ref pChip, inst,
        ...
        /*
		protected override void t進行描画_KMD_チップ_KMD_ギター_KMD_ウェイリング( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip )
		{
			...
			base.t進行描画_KMD_チップ_KMD_ギター_KMD_ウェイリング( configIni, ref dTX, ref pChip );
		}
         */
		protected override void t進行描画_KMD_チップ_KMD_フィルイン( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip )
		{
			if ( !pChip.bHit && ( pChip.nバーからの距離dot.Drums < 0 ) )
			{
				pChip.bHit = true;
					...
					case 0x02:	// フィルイン終了
                        ...
                        if (((this.actCombo.n現在のコンボ数.Drums > 0) || configIni.bドラムが全部オートプレイである) && configIni.b歓声を発声する)
                        {
                            ...
                            else
                            {
                                ...
                                CDTXMania.Skin.sound歓声音.n位置_KMD_次に鳴るサウンド = 0;
                            }
                            //if (CDTXMania.ConfigIni.nSkillMode == 1)
        ...
        protected override void t進行描画_KMD_チップ_KMD_ボーナス(CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip)
        {
        ...
        /*
		protected override void t進行描画_KMD_チップ_KMD_ベース_KMD_ウェイリング( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip )
		{
			if ( configIni.bGuitar有効 )
			{
				...
				base.t進行描画_KMD_チップ_KMD_ベース_KMD_ウェイリング( configIni, ref dTX, ref pChip);
		}
         */

        protected override void t進行描画_KMD_チップ_KMD_空打ち音設定_KMD_ドラム(CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip)
        {
            if (!pChip.bHit && (pChip.nバーからの距離dot.Drums < 0))
            {
                try
		...
		protected override void t進行描画_KMD_チップ_KMD_小節線( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip )
		{
