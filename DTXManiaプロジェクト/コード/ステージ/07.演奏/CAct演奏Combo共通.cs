...
namespace DTXMania
{
    internal class CAct演奏Combo共通 : CActivity
    {
        ...
        protected virtual void tコンボ表示_KMD_ドラム(int nCombo値, int nジャンプインデックス)
        {
        }
        protected virtual void tコンボ表示_KMD_ドラム(int nCombo値, int nジャンプインデックス, int nX中央位置px, int nY上辺位置px)
        {
        ...
        protected virtual void tコンボ表示_KMD_ギター(int nCombo値, int nジャンプインデックス)
        {
        }
        protected virtual void tコンボ表示_KMD_ベース(int nCombo値, int nジャンプインデックス)
        {
        }
        protected void tコンボ表示_KMD_ギター(int nCombo値, int nジャンプインデックス, int nコンボx, int nコンボy)
        {
            #region [ 事前チェック。]
            //-----------------
        ...
        protected void tコンボ表示_KMD_ベース(int nCombo値, int nジャンプインデックス, int nコンボx, int nコンボy)
        {
            #region [ 事前チェック。]
            //-----------------
        ...
        public override void On活性化()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                this.status[i].n前回の時刻_KMD_ジャンプ用 = -1;
        ...
        public override int On進行描画()
        {
            ...
            for (int i = 2; i >= 0; i--)
            {
                ...
                {
                    if ((this.status[i].n前回の時刻_KMD_ジャンプ用 == -1) || (CDTXMania.Timer.n現在時刻 < this.status[i].n前回の時刻_KMD_ジャンプ用))
                        this.status[i].n前回の時刻_KMD_ジャンプ用 = CDTXMania.Timer.n現在時刻;

                    const long INTERVAL = 2;
                    while ((CDTXMania.Timer.n現在時刻 - this.status[i].n前回の時刻_KMD_ジャンプ用) >= INTERVAL)
                    {
                        if (this.status[i].nジャンプインデックス値 < 2000)
                            this.status[i].nジャンプインデックス値 += 3;

                        this.status[i].n前回の時刻_KMD_ジャンプ用 += INTERVAL;
                    }
                }
            //-----------------
                #endregion
            ...
            Retry:	// モードが変化した場合はここからリトライする。
                ...
                switch (this.status[i].e現在のモード)
                {
                    ...
                    case EMode.進行表示中:
                        ...
                        {
                            case 0:
                                this.tコンボ表示_KMD_ドラム(this.status[i].nCOMBO値, this.status[i].nジャンプインデックス値);
                                break;

                            case 1:
                                this.tコンボ表示_KMD_ギター(this.status[i].nCOMBO値, this.status[i].nジャンプインデックス値);
                                break;

                            case 2:
                                this.tコンボ表示_KMD_ベース(this.status[i].nCOMBO値, this.status[i].nジャンプインデックス値);
                                break;
                        }
                        break;
                    //-----------------
