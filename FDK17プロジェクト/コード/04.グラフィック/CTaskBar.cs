...
namespace FDK
{
	...
	public static class CTaskBar
	{
		...
		/// _KMD_ABM_ACTIVATE---AppBarがアクティブになった事をシステムに通知
		/// _KMD_ABM_GETAUTOHIDEBAR---スクリーンの特定の端に関連付けられているオートハイドAppBarのハンドルを返す
		/// _KMD_ABM_GETSTATE---タスクバーがオートハイドか常に最前面のどちらの常態にあるかを返す
		/// _KMD_ABM_GETTASKBARPOS---タスクバーの使用領域を返す
		/// _KMD_ABM_NEW---新しいAppBarを登録し、システムが通知に使用するメッセージIDを指定する
		/// _KMD_ABM_QUERYPOS---AppBarのためのサイズとスクリーン位置を要求する
		/// _KMD_ABM_REMOVE---AppBarの登録を削除する
		/// _KMD_ABM_SETAUTOHIDEBAR---スクリーンの端にオートハイドAppBarを登録または削除する
		/// _KMD_ABM_SETPOS---AppBarのサイズとスクリーン座標を設定する
		/// _KMD_ABM_WINDOWPOSCHANGED---AppBarの位置が変更されたことをシステムに通知する
